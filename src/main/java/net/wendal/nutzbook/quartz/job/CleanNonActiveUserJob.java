...
public class CleanNonActiveUserJob implements Job
{
	...
	public void execute( JobExecutionContext context ) throws JobExecutionException
	{
		...
		// 24*60*60*1000L); // 一天, 测试的时候可以改成1小时之类的
		Date deadtime = new Date( System.currentTimeMillis() - 24*60*60 * 1000L ); // 一天,
																				// 测试的时候可以改成1小时之类的
		...
		Sql sql = Sqls
				.create( "delete from $user_table where id > 10 and not exists (select 1 from $user_profile_table where $user_table.id = uid ) and ct < @deadtime" );
